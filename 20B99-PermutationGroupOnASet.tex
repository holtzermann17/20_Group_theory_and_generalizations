...
\usepackage{pmmeta}
\pmcanonicalname{PermutationGroupOnASet}
\pmcreated{2013-03-22 14:01:36}
\pmmodified{2013-03-22 14:01:36}
\pmowner{bwebste}{988}
\pmmodifier{bwebste}{988}
\pmtitle{permutation group on a set}
\pmrecord{7}{35022}
\pmprivacy{1}
...
\begin{document}
A \emph{permutation group} on a set $A$ is a subgroup of the symmetric group $S_A$.

\section*{fact: conjugating stabilizer of an element by permutation produces stabilizer of permuted element}

\begin{123listcolonstyle} 
\item $A$ set
\item $a\in A$
\item $X$ permutation group on $A$
\item $\sigma\in X$
\item $\sigma \mathrm{Stab}_X(a) \sigma^{-1}=  \mathrm{Stab}_X(\sigma(a))$ 
\end{123listcolonstyle}

\section*{fact: if a permutation group acts transitively, then the intersection of conjugated stabilizers is the identity}

\begin{123listcolonstyle}  % I thought that the intersection of sets was a set :)
\item $A$ set
\item $a\in A$
\item $X$ permutation group on $A$
\item $\cap_{\sigma\in X} \sigma \mathrm{Stab}_X(a)=1$ 
\end{123listcolonstyle}
