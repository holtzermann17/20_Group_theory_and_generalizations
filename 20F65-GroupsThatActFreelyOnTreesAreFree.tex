...
\usepackage{pmmeta}
\pmcanonicalname{GroupsThatActFreelyOnTreesAreFree}
\pmcreated{2013-03-22 13:54:23}
\pmmodified{2013-03-22 13:54:23}
\pmowner{mps}{409}
\pmmodifier{mps}{409}
\pmtitle{groups that act freely on trees are free}
\pmrecord{10}{34656}
\pmprivacy{1}
\pmauthor{mps}{409}
\pmtype{Theorem}
...
\begin{document}
\PMlinkescapeword{inversions}
\begin{theorem*}
Groups that act freely and without inversions on trees are free.
\end{theorem*}

\begin{proof}
Let $\Gamma$ be a group acting freely and without inversions by graph automorphisms on a tree $T$.
Since $\Gamma$ acts freely on $T$, the quotient graph $T/\Gamma$ is well-defined, and $T$ is the universal cover of $T/\Gamma$ since $T$ is contractible.  Thus by faithfulness $\Gamma\cong\pi_1(X/\Gamma)$.  Since any graph is homotopy equivalent to a wedge of circles, and the fundamental group of such a space is free by Van Kampen's theorem, $\Gamma$ is free.
\end{proof}
%%%%%
%%%%%
\end{document}
